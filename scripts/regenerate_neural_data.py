...
def write_flashcards(decks: dict, out_dir: Path) -> tuple[int, int]:
    ...
    # ── the zh set as a BITMAP, not a list of indexes ──────────────────────────
    # The manifest is the one deck file EVERY visitor fetches, so its bytes sit on the critical
    # path and are counted by the eager payload budget. As an index array this field grows with
    # the number of TRANSLATED decks: 385 of them cost 794 gzip bytes, and a fully translated
    # corpus would cost 6,325 — enough on its own to break the budget.
    #
    # One bit per deck, in the manifest's own key order, LSB-first inside each byte. That is 366
    # bytes whatever the translation covers, and gzip loves it: 203 bytes today, 26 when every
    # deck is set. Base64 beat hex on both raw (488 vs 732) and gzip (203 vs 206), measured.
    zh_bits = bytearray((len(order) + 7) // 8)
    for k in zh_keys:
        i = order.get(k)
        if i is not None:
            zh_bits[i >> 3] |= 1 << (i & 7)
    zh_b64 = base64.b64encode(bytes(zh_bits)).decode("ascii")
    (fc_dir / "_index.json").write_text(json.dumps({
        "_meta": {
            ...
            "zh": "base64 BITMAP, one bit per deck in `decks` order, LSB-first within each "
                  "byte: bit i set means deck i has a Traditional Chinese chunk at "
                  "flashcards/zh/<fnv1a32(deckKey)>.json. A deck whose bit is clear has no "
                  "translation yet and is read from the English chunk in both languages. The zh "
                  "chunk's cards keep the ENGLISH `q` — it is the scheduling identity — and "
                  "carry the translation in `qd` (question), `a`/`d` (answer) and `mc`. Format 4 "
                  "shipped this as an index array; it grew with the translation and was replaced "
                  "because the manifest is on the eager payload budget.",
        },
