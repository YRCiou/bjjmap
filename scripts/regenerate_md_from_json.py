#!/usr/bin/env python3
"""
BJJ Map JSON to Markdown Generator
====================================
...
def _with_utm(url, system_name='', product_id=''):
    """Append BJJMap UTM params to a curated affiliate URL for the PostHog funnel.
    Never touches the vendor's existing query (e.g. ref=) — only adds utm_*."""
    if not url:
        return url
    from urllib.parse import quote
    params = ['utm_source=bjjmap', 'utm_medium=affiliate', 'utm_campaign=systems']
    if system_name:
