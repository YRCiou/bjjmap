...
export function brand({ compact = false } = {}) {
  return `<button class="brand-lockup ng-logo ${compact ? "is-compact" : ""}" type="button" data-production-selector=".ng-logo" title="Open Explorer">
    <span class="ng-brand-stack"><span class="ng-word">bjjmap<span>.org</span></span><span class="ng-kbd">${icon("search", 10)} Explore <span>/</span></span></span>
  </button>`;
}
...
export function accountBubble({ signedIn = false, open = false } = {}) {
  ...
  const authRows = signedIn
    ? `<div class="menu-row menu-row--static" data-menu-email>diogo@bjjmap.pages.dev</div>
