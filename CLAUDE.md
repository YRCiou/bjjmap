# BJJ Map — AI development guide

> **NOT IN THIS REPO (read first).** Parts of this guide describe surfaces that do not exist
> here. There is no Supabase (no auth, no cloud sync, no `window.__bjjAuth`), no PostHog or any
> other analytics (`track()` is a no-op), no Cloudflare Pages Functions, no digest Worker, no
> `.github/workflows/`, and no API keys anywhere. Anything below about sign-in, accounts, cloud
> sync, analytics events, the training-day email digest, or CI workflows does not apply.
> Progress is localStorage-only.

BJJ knowledge graph and state machine as a static site, plus one canvas game app built on it.
**Site:** https://bjjmap.pages.dev · **Repo:** https://github.com/YRCiou/bjjmap
...
- One funnel event on both surfaces: `affiliate_clickout`, delegated on `a[data-affiliate="true"]`,
  with `utm_source=bjjmap&utm_medium=affiliate&utm_campaign=systems&utm_content=<system-slug>&utm_term=<product-id>`.
...
| | |
|---|---|
| Live site | https://bjjmap.pages.dev |
| Repo | https://github.com/YRCiou/bjjmap |
