      ...
      return [
        () => {
          ...
          return async (_tree, file) => {
            ...
            // `path.resolve`, not `path.posix.join`: on Windows `file.cwd` is a backslash path,
            // which posix.join treats as ONE segment — so the leading `..` of `-d ../content`
            // ate the whole thing and left a cwd-relative `content/...`. That made every git
            // pathspec miss AND made the filesystem fallback stat a path that does not exist,
            // which is the ENOENT that killed the build. `resolve` normalises both separators
            // and the `..`, and is a no-op difference on posix.
            const fullFp = path.isAbsolute(fp) ? fp : path.resolve(file.cwd, fp)
